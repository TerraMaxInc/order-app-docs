...
The PDF-Service app requires an active token to access the github @container-registry, because it fetches the @docker-image to rebuild the app from scratch every time it's called.  The ghcr tokens expire (I like to set them to last 90 days), and must be refreshed regularly.  This is probabaly the most regular maintenance task that this whole app ecosystem requires.
