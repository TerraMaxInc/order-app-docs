...
- Step 3 requires a "PAT" (Personal Access Token).  If you have the current one saved somehwere then you can skip this step, but if you don't then you have ot create a new one; GitHb won't let you see it to recopy it.  To create one:
...
- Push the image to GCHR.  It should alreadybe tagged as needed in step 1:
    - `docker push ghcr.io/terramaxchris/pdf-service:2.1.1`
...
- If that succeeds, next enter:
    ...
    -  `--name pdf-service \`
    -  `--resource-group TestResourceGroup \`
    -  `--image ghcr.io/terramaxchris/pdf-service:2.1.1`
