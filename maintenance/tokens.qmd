...
The HubSpot Service also uses a Client Secret to gain permission to send emails on its own behalf from the `tech@terramax.com` email.   This "secret" expires every 6 months.  The value is generated from the `terramax-hubspot-service-registration` in Entra app registions, under Certificates & Secrets.  When the old Secret expires, go there and reate a new one, then paste the new value into the 'CLIENT_SECRET' env variable in both the `terramax-hubspot-service` app in Azure in prod, and in the code .env file so it will work locally in dev.
